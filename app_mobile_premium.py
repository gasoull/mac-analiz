...
.block-container{width:100%;max-width:1220px;padding-left:1.15rem;padding-right:1.15rem}
div[data-baseweb="select"]>div{border-radius:12px!important}

/* Mobile fixture cards */
.mobile-fixtures{display:none}
.m-league{margin:18px 0 8px;font-size:.9rem;font-weight:950;color:#111827}
.m-card{
  background:#fff;border:1px solid #e5e7eb;border-radius:16px;
  margin:0 0 10px;overflow:hidden;box-shadow:0 5px 18px rgba(15,23,42,.045)
}
.m-top{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:#fafbfc;border-bottom:1px solid #edf0f3}
.m-time{font-size:.76rem;font-weight:950;color:#475569}
.m-status{font-size:.68rem;font-weight:900;color:#64748b}
.m-teams{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;padding:14px 12px 12px}
.m-team{font-size:.84rem;font-weight:900;color:#172033;line-height:1.2}
.m-team.away{text-align:right}
.m-vs{font-size:.66rem;font-weight:950;color:#a0a7b2}
.m-markets{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;padding:0 10px 11px}
.m-market{background:#f7f8fa;border:1px solid #edf0f3;border-radius:10px;padding:8px 5px;text-align:center}
.m-market-name{display:block;font-size:.58rem;color:#7b8492;font-weight:850;margin-bottom:4px}
.m-market-val{font-size:.82rem;font-weight:950;color:#1f2937}
.m-market-val.strong{color:#08783e}.m-market-val.mid{color:#9a6700}

@media(max-width:768px){
  .block-container{max-width:100%!important;padding:4.2rem .72rem 4rem!important}
  [data-testid="stSidebar"]{width:min(88vw,330px)!important}

  .topbar{padding:11px 12px;min-height:auto;margin-bottom:11px;border-radius:14px;gap:9px}
  .brand-wrap{gap:9px}
  .brand-mark{width:38px;height:38px;border-radius:11px;font-size:13px}
  .brand-title{font-size:.94rem;line-height:1.08}
  .brand-sub{font-size:.62rem;margin-top:3px}
  .header-meta{display:none}

  .title{font-size:1rem;margin-top:12px}
  .subtitle{font-size:.7rem;line-height:1.35}

  /* Streamlit date navigation: 3 usable columns */
  div[data-testid="stHorizontalBlock"]:has(.stDateInput){
    gap:.38rem!important;flex-wrap:wrap!important
  }
  div[data-testid="stHorizontalBlock"]:has(.stDateInput)>div{
    min-width:0!important;flex:1 1 30%!important
  }
  div[data-testid="stHorizontalBlock"]:has(.stDateInput)>div:last-child{display:none}
  .stButton button{min-height:42px!important;font-size:.72rem!important;padding:.45rem .35rem!important}
  .stDateInput input{font-size:.72rem!important}

  /* Desktop fixture table off, purpose-built cards on */
  .desktop-fixtures{display:none!important}
  .mobile-fixtures{display:block!important}

  .fixture-league{font-size:.95rem;padding:11px 12px;margin-top:17px;border-left-width:4px}
  .premium-match-hero{margin-top:12px;border-radius:16px}
  .pmh-top{padding:9px 10px;font-size:.62rem;gap:8px}
  .pmh-main{grid-template-columns:minmax(0,1fr) 48px minmax(0,1fr);gap:6px;padding:17px 7px 19px}
  .pmh-badge{width:48px;height:48px;border-radius:14px;font-size:.82rem}
  .pmh-name{font-size:.78rem;line-height:1.15;overflow-wrap:anywhere}
  .pmh-time{font-size:1.08rem}
  .form-dot{width:18px;height:18px;border-radius:5px;font-size:.52rem}
  .pmh-form{gap:3px}

  .premium-grid{grid-template-columns:1fr 1fr;gap:8px;margin:10px 0}
  .premium-card{padding:11px;border-radius:13px}
  .pc-label{font-size:.57rem;margin-bottom:7px}
  .pc-value{font-size:1.15rem}
  .pc-sub{font-size:.62rem;line-height:1.35}
  .premium-section{padding:11px;border-radius:14px}
  .premium-two{grid-template-columns:1fr;gap:8px}
  .team-form-card{padding:10px}
  .stat-line{font-size:.7rem;padding:6px 0}

  div[data-testid="stMetric"]{padding:7px!important}
...
@media(max-width:390px){
  ...
  .premium-grid{grid-template-columns:1fr 1fr}
  .m-markets{gap:5px;padding-left:8px;padding-right:8px}
  .m-market{padding:7px 3px}
}
</style>
""", unsafe_allow_html=True)
...
if not visible_rows:
    ...
    def mobile_market(name, value):
        if value is None:
            value_text, cls = "—", ""
        else:
            value_text = f"%{value:.0f}"
            cls = "strong" if value >= 70 else ("mid" if value >= 60 else "")
        return (
            f'<div class="m-market"><span class="m-market-name">{escape(name)}</span>'
            f'<span class="m-market-val {cls}">{value_text}</span></div>'
        )

    for league_name, matches in grouped.items():
        # Desktop/tablet table
        st.markdown('<div class="desktop-fixtures">', unsafe_allow_html=True)
        st.markdown(f'<div class="fixture-league">🌐 {escape(league_name)}</div>', unsafe_allow_html=True)
        st.markdown("""
        <div class="fixture-head">
          <div>Saat</div><div>Maç</div><div>1.5 Üst</div><div>2.5 Üst</div>
          <div>3.5 Üst</div><div>3.5 Alt</div><div>KG Var</div><div>KG Yok</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Mobile cards
        cards = [f'<div class="m-league">🌐 {escape(league_name)}</div>']
        for m in matches:
            s = m["scores"]
            if m.get("status") == "FINISHED":
                status_text = f'MS • {m.get("home_score","")}-{m.get("away_score","")}'
            elif m.get("status") == "LIVE":
                status_text = f'CANLI • {m.get("live_home_score","")}-{m.get("live_away_score","")}'
            else:
                status_text = "Maç öncesi"

            markets = "".join([
                mobile_market("1.5 Üst", s.get("1.5 Üst")),
                mobile_market("2.5 Üst", s.get("2.5 Üst")),
                mobile_market("3.5 Üst", s.get("3.5 Üst")),
                mobile_market("3.5 Alt", s.get("3.5 Alt")),
                mobile_market("KG Var", s.get("KG Var")),
                mobile_market("KG Yok", s.get("KG Yok")),
            ])
            cards.append(f"""
            <div class="m-card">
              <div class="m-top">
                <span class="m-time">{escape(str(m.get("time") or "—"))}</span>
                <span class="m-status">{escape(status_text)}</span>
              </div>
              <div class="m-teams">
                <div class="m-team">{escape(m.get("home",""))}</div>
                <div class="m-vs">VS</div>
                <div class="m-team away">{escape(m.get("away",""))}</div>
              </div>
              <div class="m-markets">{markets}</div>
            </div>
            """)
        st.markdown('<div class="mobile-fixtures">' + "".join(cards) + '</div>', unsafe_allow_html=True)
